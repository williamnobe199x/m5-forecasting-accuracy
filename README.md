...
- 本地验证窗口为 `d_1914..d_1941`，与 Kaggle 官方 private/public leaderboard 不是同一个切片。
- sNaive 基线 Avg WRMSSE: `0.837726`。
- archived LightGBM 复现 Avg WRMSSE: `0.475241`。
- 优化后 Avg WRMSSE: `0.471374`。
- 优化后相对 sNaive 提升: `43.73%`。
- 候选扫描显示，当前最稳的可落地优化是全局趋势 multiplier；更细的分组 multiplier 和 horizon multiplier 在单窗口验证中没有提升。
...
- `scripts/sanitize_artifacts.py`: 清理交付物中的内部 claim tags 和聊天审计标记。
...
`run_full_pipeline.py` 会刷新优化候选、贡献分析、可视化资产和 HTML 展示页。它默认使用已经存在的 `output/archive_validation_predictions.csv` 与 `output/snaive_validation_predictions.csv`。
...
Codex 对话中可以使用内部 claim tags 来帮助校准可信度，但网页、Markdown 讲稿、README、文档和其他对外交付物不保留这些标签。

检查交付物：

```bash
python scripts/sanitize_artifacts.py --check
```
...
- 建立 rolling CV，把 multiplier、blend、direct 模型和新特征放到同一验证框架里。
- 增加 btrotta-style 聚合 lag/rolling 特征：`item_id`、`item_id + store_id`、`dept_id + store_id`。
- 增加 4 段 direct horizon LightGBM，与当前 recursive 预测做 ensemble。
- 在 CV 稳定后再引入层级 reconciliation 和时间序列 foundation model 抽样实验。
